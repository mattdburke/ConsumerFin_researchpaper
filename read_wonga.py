...
# 2. We then want them in lower case, and stripped of blank spaces either side.
...
# We define the paths to the files and additional path variables for later use

paydayhomepagedirectory = os.fsencode("C:/Users/mattb/Documents/University/PhD/Project_ConsumerFinance/Chapter_2/Paydayhomepages/")

path_paydayhome = "C:/Users/mattb/Documents/University/PhD/Project_ConsumerFinance/Chapter_2/Paydayhomepages/"

PersonalLoanhomepagedirectory = os.fsencode("C:/Users/mattb/Documents/University/PhD/Project_ConsumerFinance/Chapter_2/PersonalLoanhomepages/")

path_PersonalLoanhomepage = "C:/Users/mattb/Documents/University/PhD/Project_ConsumerFinance/Chapter_2/PersonalLoanhomepages/"

Creditdirectory = os.fsencode("C:/Users/mattb/Documents/University/PhD/Project_ConsumerFinance/Chapter_2/Credithomepages/")

path_Credit = "C:/Users/mattb/Documents/University/PhD/Project_ConsumerFinance/Chapter_2/Credithomepages/"


# 5. We store the file names and directory paths in a vector for each category of lender, 
...
# 6. This function extracts the text we are interested in from the HTML, discards everything else.
...
# 7. Here we store the text for each site in a vector. These vectors are quite large given each 
...
# 8. Here we define a range of functions to extract our variables from the text. 
	
from string import punctuation
from nltk import sent_tokenize as st
from nltk.corpus import cmudict

# 9. We choose not to use a word tokenizer, since this captures punctuation too, here we remove it all.
...
def finance_terms(text):
	a = re.findall(r"\w+(?:[-']\w+)*|'|[-.(]+|\S\w*", text, flags = re.IGNORECASE)
	return len(set(a).intersection(financewords))
	
def APR_explained_check(text):
	a = re.findall(r"annual percentage rate", text, flags = re.IGNORECASE)
	return len(a)
...
def sup_syl(text):
	syllable_count = sum(map(lambda w: max(numsyllables(w)), wt(text)))
...
# 10. Now we iterate through all of the text lists and create variables to form a PANDA
...
PDHsupercomplexwordcount = list(map(super_complex_word_counter, PDHtext))

PDHsuperfinancetocomplex = list(map(super_financial_to_complex, PDHtext))
...
PLsupercomplexwordcount = list(map(super_complex_word_counter, PLtext))

PLsuperfinancetocomplex = list(map(super_financial_to_complex, PLtext))
...
Csupercomplexwordcount = list(map(super_complex_word_counter, Ctext))

Csuperfinancetocomplex = list(map(super_financial_to_complex, Ctext))
...
#path='C:\\Users\\mattb\\Documents\\University\\PhD\\Project_ConsumerFinance\\'

#PDHFrame.to_csv(path+'paydayhome.csv')

#PLFrame.to_csv(path+'personalloanhome.csv')

#CFrame.to_csv(path+'credithome.csv')
